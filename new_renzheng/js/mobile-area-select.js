		...
		var _private={
			...
			createHtml:function(){
				var str='<div class="select-dialog show" id="'+settings.id+'">'
					...
					str=str+'</ul><p></p></div>'
						+'<div class="select-action"><button class="js-close b-close" style="position:absolute;left:5px;width:2rem;height:1.5rem;display:block;font-size:1rem;">取消</button><button class="js-submit b-submit" style="position:absolute;right:5px;width:2rem;height:1.5rem;display:block;font-size:1rem;">确定</button></div>'
						+'</div></div>';
				this.htmlStr=str;
			...
			f: function(data) {
				...
				var str = '<dl><dd class="ddt" ref="0">省份</dd>';
				var focus = 0,
					childData, top = _this.mtop;
					if (_this.index !== 0 && _this.value[_this.index - 1] == "0") {
						$(".ddt").text("地市");
						$(".ddt").eq(0).text("省份");
						// str = '<dl><dd ref="0" class="focus">地市</dd>';
						...
						// 	str = '<dl><dd ref="0" class="focus">不限</dd>';
						// 	focus = 0;
						// }
			...
			submit: function() {
				...
				if(this.oldvalue[0]=="0"){
					$(".cityfk").css("display","block");
					$(".mengceng").css("display","block");
				}else{
					if(this.oldvalue[1]=="0"){
						$(".cityfk").css("display","block");
						$(".mengceng").css("display","block");
					}else{
						this.close();
